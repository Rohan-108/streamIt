...
export const ChatInfo = ({ isDelayed, isFollowersOnly }: ChatInfoProps) => {
  ...
  return (
    <div className="p-2 text-muted-foreground bg-white/5 border border-white/10 w-full rounded-t-md flex items-centergap-x-2">
      <Hint label={hint}>
        <Info className="h-4 w-4" />
      </Hint>
      <p className="text-ws font-semibold">{label}</p>
    </div>
  );
};
